# Импортируем модуль на написания unit-тестов:
import unittest
...
class Tictac:
    ...
    def __init__(self, players=2, size=3):
        ...
        # Набор координат пустых клеток игрового поля:
        self.empty = set([(i % size, i // size) for i in range(size * size)])
        # Наборы ходов для каждого игрока:
    ...
    # Функция, реализующая переключение на следующего "текущего" игрока:
    def next_player(self, player):
        return (player + 1) % self.players

    # Функция проверки наличия победителя игры:
    ...
    def sprint_result(self):
        result = []
        # Условие - если игра не завершена:
        if self.state != Tictac.ST_OK:
